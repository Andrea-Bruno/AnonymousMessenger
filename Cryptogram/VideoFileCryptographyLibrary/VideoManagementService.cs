    ...
    public static class VideoManagementService
    {
        ...
        public static async Task<string> DownloadVideo(byte[] _privateKey, DownloadStatus downloadStatus)
        {
            ...
            if (fileList.Count > 0)
            {
                ...
                string folder = Path.Combine(Xamarin.Essentials.FileSystem.AppDataDirectory, "downloads");
                if (!Directory.Exists(folder))
                    Directory.CreateDirectory(folder);
                string videoFile = Path.Combine(folder, _fileNameWithType);
                ...
                using (FileStream outputStream = File.OpenWrite(videoFile))
                {
                    ...
                    for (; chunk < fileList.Count; chunk++)
                    {
                        ...
                        double progressPercentAge = Math.Ceiling((chunk / Convert.ToDouble(fileList.Count)) * 100);
        ...
        public static async Task<byte[]> UploadVideo(FileResult selectedFile, UploadStatus uploadStatus, byte[] thumbnail, CancellationToken token)
        {
            ...
            try
            {
                ...
                using (var fileStream = await selectedFile.OpenReadAsync().ConfigureAwait(false))
                {
                    ...
                    try
                    {
                        string folder = Path.Combine(FileSystem.AppDataDirectory, "uploads");
                        if (!Directory.Exists(folder))
                        {
                            Directory.CreateDirectory(folder);
                        }
                        ...
                        if (File.Exists(localFilePath))
                        {
                            ...
                            using (FileStream local = File.OpenRead(localFilePath))
                            {
