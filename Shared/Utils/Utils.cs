...
namespace Utils
{
    public static class Utils
    {
        ...
        {
            var _latLng = lng.ToString(CultureInfo.InvariantCulture) + "," + lat.ToString(CultureInfo.InvariantCulture);
        ...
        public static void HandleButtonSingleClick(this object sender, int duration = 1000)
        {
            ...
            if (Device.RuntimePlatform != Device.UWP)
            {
                try
                {
                    ...
                    var timer = new System.Threading.Timer((object obj) => Device.BeginInvokeOnMainThread(() => ((View)sender).IsEnabled = true), null, duration, System.Threading.Timeout.Infinite);
                }
                catch (Exception e)
                {
