...
namespace XamarinShared.ViewCreator
{
    public abstract class BaseView : ReplySwipeLayout
    {
        ...
        protected void Init(MessageReadStatus readStatus, Func<Contact, MessageType, Task> sendNotification)
        {
            var messageStatus = readStatus.AddReadStatusFlag(Message, IsMyMessage, sendNotification, out FlagAndTime);
        ...
        private void FlagAndTime_SizeChanged(object sender, EventArgs e)
        {
            ...
            if (MessageTypeList.Contains(Message.Type))
            {
                ...
                for (var i = isReplyLayoutLoaded ? 1 : 0; i < MessageFrameContent.Children.Count - 1; i++)
                {
                    var view = MessageFrameContent.Children[i];
