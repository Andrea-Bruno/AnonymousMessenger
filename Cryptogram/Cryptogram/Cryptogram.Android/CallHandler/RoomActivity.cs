...
namespace Telegraph.Droid.CallHandler
{
    ...
    public class RoomActivity : Activity
    {
        ...
        private void InitAgoraEngineAndJoinChannel()
        {
            ...
            var _videoCallEnable = Intent.GetBooleanExtra("videoCallEnable", false);
            if(!_videoCallEnable)
                MuteLocalVideoStream(_videoCallEnable);
            isInitialized = true;
        }
        ...
        public void OnLocalVideoMuteClicked(View view)
        {
            ...
            MuteLocalVideoStream(iv.Selected);
        }

        void MuteLocalVideoStream(bool isVideoCall)
        {
            ImageView cameraButton = FindViewById<ImageView>(Resource.Id.mute_video_button);
            if (isVideoCall)
            {
                cameraButton.Selected = false;
                cameraButton.SetImageResource(Resource.Drawable.ic_cam_active_call);
            }
            else
            {
                cameraButton.Selected = true;
                cameraButton.SetImageResource(Resource.Drawable.ic_cam_disabled_call);
            }
            AgoraEngine.MuteLocalVideoStream(cameraButton.Selected);
            _isVideoEnabled = !cameraButton.Selected;
            FindViewById(Resource.Id.local_video_container).Visibility = cameraButton.Selected ? ViewStates.Visible : ViewStates.Gone;
