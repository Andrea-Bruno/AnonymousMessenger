...
{
    public partial class SendContactPage : BasePage
    {
...
#pragma warning disable CS0414 // Il campo 'SendContactPage._isEditMode' è assegnato, ma il suo valore non viene mai usato
        private readonly bool _isEditMode = false;
#pragma warning restore CS0414 // Il campo 'SendContactPage._isEditMode' è assegnato, ma il suo valore non viene mai usato
        private readonly Contact _contact;

        public SendContactPage() => InitializeComponent();

        public SendContactPage(List<Contact> participants, Contact contact)
        {
            InitializeComponent();
        ...
        private async void Send_Clicked(object sender, EventArgs args)
        {
            if (_selectedUsers.Count() >= 1)
                await Navigation.PushAsync(new SendMoneyUsersPage(_selectedUsers), false);
            else
