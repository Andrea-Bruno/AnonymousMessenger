...
namespace VideoFileCryptographyLibrary
{
    public static class FileService
    {
        ...
        public static async Task<string> UploadFile(Stream fileStream)
        {
            ...
            string _FileName = ComputePsuedoHash.ToHex(ComputePsuedoHash.GetHash256(ComputePsuedoHash.GetHash256(fileStream)));
            EncryptionService Encryptor = new EncryptionService(ComputePsuedoHash.HexToByteArray(_FileName));
            ...
            if (fileList.Count == 0)
            {
                int FSBytes = (int)fileStream.Length;
                ...
                if (!(div == 0 && mod == 0))
                {
                    ...
                    for (; i < div; i++)
                    {
                        ...
                        byte[] encryptedData = Encryptor.Encrypt(chunk);
                        //Upload file to Server
                        string filename = string.Format("{0}-{1}.mp4", i, _FileName);
        ...
        public static async Task<bool> UploadFileChunk(Stream fileStream, string fileName, byte[] privateKey, int ChunkNumber = 0, int LastChunkSize = 0)
        {
            ...
            try
            {
                EncryptionService Encryptor = new EncryptionService(privateKey);
                byte[] chunk;
                if (LastChunkSize == 0)
        ...
        {
            EncryptionService Decryptor = new EncryptionService(privateKey);
            try
            {
