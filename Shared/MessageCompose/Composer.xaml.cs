...
namespace MessageCompose
{
    ...
    public partial class Composer : BaseContentView
    {
        ...
        public delegate void MediaFileSelectEvent(List<byte[]> data, ulong? replyToPostId = null);
        ...
        private void Finish_Recording_Clicked(object sender, EventArgs e)
        {
            ...
            audioOutput = DependencyService.Get<IAudioRecorder>().GetOutput();
            UIState = 3;
        ...
        private async Task OpenEditImagePageAsync(FileResult file)
        {
            if (file != null)
            {
                ...
                _onMediaFileSelected(new List<byte[]>() { Utils.Utils.StreamToByteArray(stream) }, ReplyToPostId);
            }
