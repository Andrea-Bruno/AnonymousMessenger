...
namespace TelegraphXamarinShared
{
	public class MessageReadStatus
	{
		public MessageStatus AddReadStatusFlag(Message message, bool isMyMessage, Func<Contact, MessageType, Task> SendNotificationToChannel, out StackLayout flagAndTime)
		{
			...
			var timeLabel = new Label() { FontSize = 9, FontFamily = "LatoRegular" };
			var differentDay = Time.CurrentTimeGMT.ToLocalTime().DayOfYear == message.Creation.ToLocalTime().DayOfYear;
			timeLabel.Text = differentDay
				? message.Creation.ToLocalTime().ToShortTimeString()
				: message.Creation.ToLocalTime().ToLongDateString();

			flagAndTime = new StackLayout() { Orientation = StackOrientation.Horizontal, HorizontalOptions = isMyMessage ? LayoutOptions.EndAndExpand : LayoutOptions.StartAndExpand };
