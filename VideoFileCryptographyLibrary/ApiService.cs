...
namespace VideoFileCryptographyLibrary
{
    internal static class ApiService
    {
        ...
        public static async Task<bool> UploadFileAsync(Stream sourceStream, string fileName)
        {
            ...
            var uploadUrl = string.Format("{0}Upload?fileName={1}", Url, fileName);
        ...
        {
            var targetUrl = string.Format("{0}FileExists?fileName={1}", Url, fileName);
            using (var client = new HttpClient())
            {
                try
                {
        ...
        {
            var downloadUrl = string.Format("{0}Download?filename={1}", Url, fileName);
        ...
        {
            sourceStream.Position = 0;
            var uploadUrl = string.Format("{0}UploadThumbnail?fileName={1}", Url, fileName);
            var fileStreamContent = new StreamContent(sourceStream);
            fileStreamContent.Headers.ContentDisposition = new ContentDispositionHeaderValue("form-data") { Name = "fileName", FileName = fileName };
            fileStreamContent.Headers.ContentType = new MediaTypeHeaderValue("application/octet-stream");
            using (var client = new HttpClient())
        ...
        {
            var downloadUrl = string.Format("{0}DownloadThumbnail?filename={1}", Url, fileName);
            using (var client = new HttpClient())
            {
                try
                {
