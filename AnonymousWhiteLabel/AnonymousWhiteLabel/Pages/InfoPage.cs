...
namespace AnonymousWhiteLabel.Pages
{
    public class InfoPage : ContentPage
    {
        ...
        public InfoPage()
        {
            ...
            avatar.Source = ImageSource.FromStream(() => new MemoryStream(App.Context.My.GetAvatar()));
            ...
            App.Context.My.OnAvatarChanged += (imageData) =>
            {
                avatar.Source = ImageSource.FromStream(() => new MemoryStream(imageData));
            };
            name.TextChanged += (sender, e) =>
                {
                    App.Context.My.SetName(name.Text);
                };
            ...
            openSecure.Clicked += async (sender, e) =>
            {
                await Browser.OpenAsync("https://github.com/BitBoxLab-OU/Secure-Storage", BrowserLaunchMode.SystemPreferred);
            };
            ...
            openCrypto.Clicked += async (sender, e) =>
            {
                await Browser.OpenAsync("https://github.com/BitBoxLab-OU/Crypto-Messenger", BrowserLaunchMode.SystemPreferred);
            };
            ...
            foreach (Suggestion suggestion in suggestions)
            {
                ...
                openSuggestion.Clicked += async (sender, e) =>
                {
                    await Browser.OpenAsync(suggestion.Link, BrowserLaunchMode.SystemPreferred);
                };
            ...
            ScrollView scrollView = new ScrollView
            {
                ...
                Content = new StackLayout { Margin = new Thickness(5), Orientation = StackOrientation.Vertical, Children = { avatar, upload, nameLabel, name, labelPublicKey, publicKey, qrImage, sharePublicKeyButton, showPassphrase, secureLabel, openSecure, cyptoLabel, openCrypto, suggestLabel, suggestGrid } },
            };
