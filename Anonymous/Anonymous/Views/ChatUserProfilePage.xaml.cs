...
namespace Telegraph.Views
{
    ...
    public partial class ChatUserProfilePage : BasePage
    {
        ...
        {
            if (MessageConfirmationButton.IsOn == true)
            {
                _contact.SendConfirmationOfReading = true;
                _contact.Save();
            }
            else
            {
                _contact.SendConfirmationOfReading = false;
                _contact.Save();
            }
        }
        ...
        private void Message_Clicked(object _, EventArgs e) => Application.Current.MainPage.Navigation.PushAsync(new ChatRoom(_contact), false);
