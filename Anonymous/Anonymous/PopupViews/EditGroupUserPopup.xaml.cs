...
namespace Telegraph.Views
{
	...
	public partial class EditGroupUserPopup : BasePopupPage
	{
		...
		private async void Message_Clicked(object _, EventArgs e) => 
			await Navigation.PushAsync(new ChatRoom(_contact), false);

		private async void View_Clicked(object _, EventArgs e) => 
			await Navigation.PushAsync(new ChatUserProfilePage(_contact), false);

		private void Background_Clicked(object _, EventArgs e) => OnBackButtonPressed();
