...
namespace Telegraph.Views
{
    ...
    {
        private Utils.FontSizeConverter _fontSizeConverter;
        private int _textSize=1;
                                                
        public float TextSize
        {
            get { 
                return Defaults.FontRatios[_textSize];
            }
            set {
                _textSize = (int)value;
        ...
        public TextSizePage()
        {
            ...
            _fontSizeConverter = new Utils.FontSizeConverter();
        ...
        {
            Defaults.DefaultSelectedFontRatio = Defaults.FontRatios[_textSize];
            Preferences.Set("FontRatio", Defaults.DefaultSelectedFontRatio);
            XamarinShared.ViewCreator.MessageViewCreator.Instance.TextSize = Defaults.DefaultSelectedFontRatio;
            OnBackButtonPressed();
        }
     
        private int FindFontRationIndex()
        {
            for (int i = 0; i < Defaults.FontRatios.Length; i++)
                if (Defaults.DefaultSelectedFontRatio == Defaults.FontRatios[i])
                    return i;
            return 1;
        }
